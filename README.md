...
基于 [dushixiang/uart_sms_forwarder](https://github.com/dushixiang/uart_sms_forwarder) 扩展的 Air780 系列短信与来电管理平台。本版本增加多模块统一管理、按模块收发短信、通话记录、定时流量保活和可配置的无应答呼叫转移。

> 当前平台发布版：`v1.3.0-multi.1`
> Lua 插件版本：`1.3.0`
...
| 模块 1 | Air780EPV、当前设备 Lua 插件 `1.2.2` | 在线、状态读取及流量任务通过 |
| 模块 2 | Air780EHV、`LuatOS-SoC_V2036_Air780EHV_116.soc`、当前设备 Lua 插件 `1.2.2` | 在线、状态读取及流量任务通过 |
| 多模块 | 两个 USB 串口同时运行 | `2/2` 在线 |
| EHV 来电 | 真实来电、挂断事件及 5 秒持续时长 | 数据库、API、桌面和手机页面均通过 |
| 平台 `v1.3.0-multi.1` | 双模块状态、来电记录、无应答转移配置与数据库迁移 | 通过 |
| Lua `1.3.0` | EPV、EHV 和通用脚本语法及受限协议 | 通过；真实运营商转移待刷写后验证 |
...
三个脚本的功能版本均为 `1.3.0`。请先选择与硬件型号完全匹配的 LuatOS 底层固件，再用 LuaTools 写入对应 `main.lua`。不要把 Air780EHV 的 `.soc` 底层固件写入其他型号。

平台升级、网页修改或数据库迁移不要求重新刷写插件。只有插件版本变化时才需要重新写入模块。

从 `1.2.2` 升级到 `1.3.0` 时，Air780EPV 和 Air780EHV 都需要各写入一次对应目录里的 `main.lua`。写入后，串口控制页应显示插件版本 `1.3.0` 和“插件支持：支持”。
...
- Docker Hub：`s121934/uart_sms_forwarder_multi:1.3.0-multi.1`
- GHCR：`ghcr.io/xxs-dev/uart_sms_forwarder_multi:1.3.0-multi.1`
